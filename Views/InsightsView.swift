...
struct InsightBarChart: View {
            ...
            HStack(alignment: .top, spacing: 0) {
                ...
                if isScrollable {
                    ScrollView(.horizontal, showsIndicators: false) {
                        barsView(barWidth: barWidth, spacing: spacing, maxVal: maxVal,
                                 height: geo.size.height, barAvailH: barAvailH)
    ...
    private func barsView(barWidth: CGFloat, spacing: CGFloat, maxVal: Double,
                          height: CGFloat, barAvailH: CGFloat) -> some View {
        HStack(alignment: .bottom, spacing: spacing) {
            ForEach(Array(data.enumerated()), id: \.element.id) { index, point in
                let value = showMinutes ? point.minutes : Double(point.trackCount)
                let barHeight = max(barAvailH * CGFloat(value / maxVal), 4)
    ...
    private func yAxisTicks(maxVal: Double) -> [Double] {
        ...
        let interval = tickInterval(forMax: maxVal)
        return stride(from: 0.0, through: maxVal + 0.001, by: interval).map { $0 }
    }
...
// MARK: - Shared Genre Colors

func genreColor(at index: Int) -> Color {
    switch index {
    case 0: return Color.emerald600
    ...
    case 4: return Color(red: 0.3, green: 0.4, blue: 0.9)
    case 5: return Color(red: 0.0, green: 0.7, blue: 0.7)
    case 6: return Color(red: 0.9, green: 0.3, blue: 0.4)
    case 7: return Color(red: 0.2, green: 0.6, blue: 0.9)
    case 8: return Color(red: 0.8, green: 0.4, blue: 0.6)
    case 9: return Color(red: 0.4, green: 0.7, blue: 0.5)
    default:
...
struct ListeningInsightTab: View {
        ...
        return VStack(spacing: 0) {
            VStack(spacing: Spacing.spacing12) {
                ...
                VStack(spacing: Spacing.spacing4) {
                    ...
                    Text("\(formatNumber(Int(totalMinutes))) min")
            ...
            HStack(spacing: 0) {
                ...
                if viewModel.selectedPeriod != .day {
                    ...
                    VStack(spacing: 2) {
                        let avgPerDay = totalMinutes / Double(viewModel.daysInPeriod)
    ...
    @ViewBuilder
    private var genrePieChartSection: some View {
        if let genres = viewModel.stats?.topGenres, !genres.isEmpty {
            let displayGenres = Array(genres.prefix(5))
            let total = displayGenres.reduce(0) { $0 + $1.playCount }
            ...
            VStack(alignment: .leading, spacing: Spacing.spacing12) {
                ...
                HStack(spacing: Spacing.spacing16) {
                    ZStack {
                        ForEach(Array(displayGenres.enumerated()), id: \.element.genre) { index, genre in
                            let startAngle = displayGenres.prefix(index).reduce(0.0) { $0 + Double($1.playCount) / Double(max(total, 1)) * 360 }
                            let endAngle = startAngle + Double(genre.playCount) / Double(max(total, 1)) * 360
                            PieSlice(startAngle: .degrees(startAngle - 90), endAngle: .degrees(endAngle - 90))
                                .fill(genreColor(at: index))
                        }
                        Circle().fill(Color.surfaceCard).frame(width: 60, height: 60)
                        VStack(spacing: 0) {
                            Text("\(displayGenres.count)")
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ForEach(Array(displayGenres.enumerated()), id: \.element.genre) { index, genre in
                            HStack(spacing: 8) {
                                Circle().fill(genreColor(at: index)).frame(width: 8, height: 8)
                                Text(genre.genre.capitalized)
                                    .font(.system(size: 11))
                                    .foregroundColor(.emerald900)
                                    .lineLimit(1)
                                Spacer()
                                Text("\(Int(Double(genre.playCount) / Double(max(total, 1)) * 100))%")
                                    .font(.system(size: 11, weight: .medium))
                                    .foregroundColor(.textSecondary)
                            }
                        }
                        ...
                        ZStack {
                            if let imageUrl = artist.imageUrl, let url = URL(string: imageUrl) {
                                AsyncImage(url: url) { image in
                                    ...
                                    Circle().fill(Color.emerald50)
                                }
                            } else {
                                Circle().fill(Color.emerald50)
                                    .overlay(Image(systemName: "person.fill").foregroundColor(.emerald400))
                            }
                        }
                        .frame(width: 40, height: 40)
                        .clipShape(Circle())
